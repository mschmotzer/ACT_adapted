...
class DETRVAE(nn.Module):
    ...
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names,velocity_control = False):
    ...
    def forward(self, qpos, image, env_state, qvel=None, actions=None, is_pad=None):
        ...
        if self.backbones is not None:
            ...
            if qvel is not None:
                # Concatenate qpos and qvel for richer proprioceptive input
                proprio_input_vel = self.input_proj_robot_state_vel(qvel)
            else:
            ...
            hs = self.transformer(src, None, self.query_embed.weight, pos, latent_input, proprio_input, self.additional_pos_embed.weight, velocity_input = proprio_input_vel)[0]
        else:
...
class CNNMLP(nn.Module):
    ...
    def forward(self, qpos, image, env_state, actions=None):
        ...
        features = torch.cat([flattened_features, qpos], axis=1) # qpos: 8
